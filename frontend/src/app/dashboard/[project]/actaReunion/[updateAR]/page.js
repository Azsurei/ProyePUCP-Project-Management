...
export default function editarActaReunion(props) {
    ...
    const { sendNotification, relistNotification } =
    ...
    useEffect(() => {
        if (actaReunion && actaReunion.lineaActaReunion) {
            ...
            if (actaReunion.lineaActaReunion.fechaReunion) {
                setDateValue(
                    actaReunion.lineaActaReunion.fechaReunion.split("T")[0]
                );
            }

            if (actaReunion.lineaActaReunion.horaReunion) {
                setTimeValue(
                    actaReunion.lineaActaReunion.horaReunion.substring(0, 5)
                );
            }
            ...
            if (actaReunion.lineaActaReunion.temasReunion) {
                const temasConData =
                    actaReunion.lineaActaReunion.temasReunion.map(
                        (tema, index) => ({
                            ...tema,
                            index: index + 1,
            ...
            if (actaReunion.lineaActaReunion.comentarios) {
                const comentariosConData =
                    actaReunion.lineaActaReunion.comentarios.map(
                        (comentario, index) => ({
                            ...comentario,
                            index: index + 1,
    ...
    const handleAsistenciaChange = (participante) => {
        const nuevosParticipantes = participantsList.map((item) => {
    ...
    const saveMeetingChanges = () => {
        ...
        axios
            ...
            .then(function (response) {
                ...
                console.log("Conexion correcta, AR editada");

                axios
                    .post(
                        process.env.NEXT_PUBLIC_BACKEND_URL +
                            "/api/usuario/actualizaNotificacionAR",
                        {
                            idOld: idLineaActaReunion,
                            idNew: response.data.idLineaActaReunion,
                        }
                    )
                    .then(function (response) {
                        console.log(response);
                        console.log("Notificacion actualizada");
                        //relistamos silenciosamente las notificaciones en las pantallas de los usuarios 
                        // for(const usuario of participantes){
                        //     if(usuario.idUsuario !== sessionData.idUsuario){
                        //         relistNotification(usuario.idUsuario);
                        //         console.log("Actualizando notificacion de " + usuario.idUsuario);
                        //     }
                        // }
                        //no funciona porque no manejan idUsuario en sus participantes. pi.pi.pi
                    })
                    .catch(function (error) {
                        console.log(error);
                    });
        ...
        <div className="newMeetingArticle">
            ...
            <HeaderWithButtons
                ...
                breadcrump={
                    "Inicio / Proyectos / " +
                    projectName +
                    " / Acta de Reunion / Editar Reunion"
                }
                btnText={"Volver"}
            >
            ...
            <div className="flex align-end">
                {!isEditMode && (
                    <button className="btnEditar" onClick={toggleEditMode}>
                        Editar
                    </button>
                )}
            </div>
                ...
                <div className="mainInfo">
                    <div className="p-5 pt-3 border border-slate-300 shadow-md relative rounded-lg">
                        <div>
                            ...
                            <p className=" convenourTitle mt-5 mb-1 font-medium">
                            ...
                            */}
                            ...
                            <div className="dateAndTimeLine flex items-center gap-10">
                                <div className="dateShow">
                                    <p className=" dateShowTitle mt-5 font-medium">
                                        Fecha
                                    </p>
                                        ...
                                        actaReunion.lineaActaReunion && (
                                            <input
                                                ...
                                                value={
                                                    isEditMode
                                                        ? dateValue
                                                        : actaReunion.lineaActaReunion.fechaReunion.split(
                                                              "T"
                                                          )[0]
                                                }
                                ...
                                <div className="timeShow">
                                    <p className="timeShowTitle mt-5 ml-5 font-medium">
                                        Hora
                                    </p>
                                        ...
                                        actaReunion.lineaActaReunion && (
                                            <input
                                                ...
                                                value={
                                                    isEditMode
                                                        ? timeValue
                                                        : actaReunion.lineaActaReunion.horaReunion.substring(
                                                              0,
                                                              5
                                                          )
                                                }
                    ...
                    <div className="mx-auto">
                        ...
                        <div className="py-0 mt-0 ml-2">
                            <div className="flex justify-between mb-2">
                                <span className="text-mg font-semibold">
                                    Lista de Miembros
                                </span>
                                <span className="text-mg font-semibold mr-6">
                                    Asistencia
                                </span>
                            </div>
