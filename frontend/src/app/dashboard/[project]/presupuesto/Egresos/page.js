...
export default function Egresos(props) {
    ...
    const addEstimacionesList = (lineasEstimacion) => {
        setlistEstimacionesSelected([lineasEstimacion]);
    };

    const removeEstimacionesInList = (lineasEstimacion) => {
        const newEstimacionList = listEstimacionesSelect.filter(
            (item) =>
                item.idLineaEstimacion !== lineasEstimacion.idLineaEstimacion
        );
    ...
    function insertatLineaEgreso() {
        return new Promise((resolve, reject) => {
            ...
            const stringUrlTipoTransaccion =
                process.env.NEXT_PUBLIC_BACKEND_URL +
                `/api/proyecto/presupuesto/insertarLineaEgreso`;
        ...
        const fetchData = async () => {
            if (presupuestoId !== "") {
                try {
                    const response = await axios.get(
                        process.env.NEXT_PUBLIC_BACKEND_URL +
                            `/api/proyecto/presupuesto/listarLineasEstimacionCostoXIdPresupuesto/${presupuestoId}`
                    );
                    ...
                    console.log(
                        `Datos obtenidos exitosamente:`,
                        response.data.lineasEstimacionCosto
                    );
                } catch (error) {
                    console.error(
        ...
        const fetchData = async () => {
            if (presupuestoId !== "") {
                try {
                    const response = await axios.get(
                        process.env.NEXT_PUBLIC_BACKEND_URL +
                            `/api/proyecto/presupuesto/listarLineasEgresoXIdPresupuesto/${presupuestoId}`
                    );
    ...
    const filteredEgresos = React.useMemo(() => {
        ...
        if (hasSearchFilterEgreso) {
            filteredEgresos = filteredEgresos.filter((item) =>
                item.descripcion
                    .toLowerCase()
                    .includes(filterEgreso.toLowerCase())
            );
        }
        ...
        <div className="mainDivPresupuesto">
            ...
            <div className="presupuesto">
                ...
                <div className="buttonsPresu">
                    ...
                    >
                        <button
                            className="btnCommon btnIngreso  sm:w-1 sm:h-1"
                            type="button"
                        >
                            Ingresos
                        </button>
                    ...
                    >
                        <button
                            className="btnCommon btnEgreso btnDisabled btnSelected sm:w-1 sm:h-1"
                            disabled
                            type="button"
                        >
                            Egresos
                ...
                <ModalContent>
                    {(onClose) => {
                        const cerrarModal = async () => {
                            ...
                            if (
                                parseInt(cantRecurso) < 0 ||
                                isNaN(parseInt(cantRecurso))
                            ) {
                            ...
                            if (Isvalid === true) {
                                try {
                                    ...
                                    console.error(
                                        "Error al registrar la línea de estimación o al obtener los datos:",
                                        error
                                    );
                                }
                                ...
                                <ModalBody>
                                    ...
                                    >
                                        <Input
                                            ...
                                            placeholder={
                                                dataLineaEstimacion.cantidadRecurso
                                            }
                                            defaultValue={
                                                dataLineaEstimacion.cantidadRecurso
                                            }
                                    ...
                                    >
                                        <input
                                            ...
                                            defaultValue={
                                                dataLineaEstimacion.fechaInicio
                                            }
                                        />
